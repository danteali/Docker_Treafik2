...
echo "Triggering Sonarr download directory scan via Sonarr API..."
...
#curl $URL/api/command -X POST -d '{"name": "downloadedepisodesscan", "path": "/downloads/"}' --header "X-Api-Key:$APIKEY"
#curl $URL/api/command -X POST -d '{"name": "downloadedepisodesscan"}' --header "X-Api-Key:$APIKEY"


curl $URL/api/command?apikey=$APIKEY \
  --insecure \
  --header "Content-Type: Application/JSON" \
  --request POST \
