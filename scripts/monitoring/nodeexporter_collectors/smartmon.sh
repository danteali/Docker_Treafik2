#!/usr/bin/env bash
# This is a customised version of 'v3' (see _archive folder).
# CUSTOMISATIONS (original code commented out below)
# - Editted script to make compatible with pre-release versions of smartctl
#   Search for 'smartctl_version_number' for changes.
# - Added 'host_system=$HOSTNAME' to all metrics since we may be running on multiple systems.
#   Search for 'host_system=\"$HOSTNAME\"' to find changes.
...
parse_smartctl_attributes() {
  local disk="$1"
  local disk_type="$2"
  local labels="host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${disk_type}\""
...
parse_smartctl_info() {
  ...
  echo "device_info{host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${disk_type}\",vendor=\"${vendor}\",product=\"${product}\",revision=\"${revision}\",lun_id=\"${lun_id}\",model_family=\"${model_family}\",device_model=\"${device_model}\",serial_number=\"${serial_number}\",firmware_version=\"${fw_version}\"} 1"
  echo "device_smart_available{host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${disk_type}\"} ${smart_available}"
  echo "device_smart_enabled{host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${disk_type}\"} ${smart_enabled}"
  [[ "${smart_healthy}" != "" ]] && echo "device_smart_healthy{host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${disk_type}\"} ${smart_healthy}"
}
...
echo "smartctl_version{host_system=\"$HOSTNAME\",version=\"${smartctl_version}\"} 1" | format_output

# Doesn't work for pre-release versions e.g. 'pre-7.5'
#if [[ "$(expr "${smartctl_version}" : '\([0-9]*\)\..*')" -lt 6 ]]; then
#  echo "smartctl version is below 6 - exiting without analysis"
#  exit
#fi

# Replacement which works with pre-release versions
smartctl_version_number=$(echo "$smartctl_version" | sed 's/[^0-9.]//g')
# this expression is cleaner but can't guarantee bc is installed
#if (( $(echo "$float_number > $integer_number" | bc -l) )); then
# From Bing AI!
if awk -v float="$smartctl_version_number" -v testint="6" 'BEGIN { exit !(float < testint) }'; then
  echo "smartctl version is below 6 - exiting without analysis"
  exit
fi
...
for device in ${device_list}; do
  ...
  echo "smartctl_run{host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${type}\"}" "$(TZ=UTC date '+%s')"
  ...
  echo "device_active{host_system=\"$HOSTNAME\",disk=\"${disk}\",type=\"${type}\"}" "${active}"
