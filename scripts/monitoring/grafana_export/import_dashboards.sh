...
#(FILE='nginx_container.json'; \
#  USERNAME="<USERNAME>"; PASSWORD="<PASSWORD>"; IP="<IP_ADDRESS"> PORT=<PORT>; \
#  HOST="http://$USERNAME:$PASSWORD@$IP:$PORT"; \
#  curl -k -X POST -H "Content-Type: application/json" -H "Accept: application/json" --data-binary @$FILE "$HOST/api/dashboards/db" )
# ...
# Or after we removed complex chars from PASS for other reasons we can use:
#curl -k -X POST -H "Content-Type: application/json" -H "Accept: application/json" --data-binary @docker-containers.json "$HOST/api/dashboards/db"
...
PORT=${secrets[PORT]}
#HOST="http://$USERNAME:$PASSWORD@$IP:$PORT"
FILETEMP="/tmp/grafana_dashboard_import.tmp"
...
for file in $DIR_DATA/* $DIR_DATA/**/*; do

    if [[ ${file} == *".json" ]]; then
        
        echo "Processing: ${file} ..."
        # Need to remove 'id' line from dashboard since Grafana will give it an ID on import
        # We used to do this in export script
        sed '0,/"id"/{/"id"/d;}' "${file}" > "$FILETEMP"

        curl -k -X POST "$HEADERAUTH" -H "Content-Type: application/json" -H "Accept: application/json" --data-binary @"$FILETEMP" "${HOST}/api/dashboards/db"
