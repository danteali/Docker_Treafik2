...
REMOTE_HOST="pihole"    # Must be defined in /etc/ssh/ssh_config with SSH key and user
...
rsync --archive --compress --info=progress2 --stats --human-readable --itemize-changes --partial --acls --xattrs --sparse --timeout=180 --delete -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" --rsync-path="sudo rsync" pihole:/var/spool/cron/crontabs/root .
