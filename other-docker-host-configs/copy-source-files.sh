...
# REMOTE_HOST can be specified here. If not defined here it MUST be specified on the command line.
# Must also be defined in /etc/ssh/ssh_config with SSH key and user to enable authentication.
REMOTE_HOST=""
...
function parse-input() {
    ...
    # Check REMOTE_HOST has been defined, if not then exit with error
    if [ -z "$REMOTE_HOST" ]; then
        echo; echo "Error: No remote host defined!"
        echo "Define REMOTE_HOST in the script variables section or specify on command line with --remote-host=HOSTNAME"
        echo "Use --help or -h to see usage information."
        echo
        exit 1
    fi
